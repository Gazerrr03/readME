...
当前视觉主题已经从 blueprint 转为深靛蓝的像素怀旧 OS：背景由经过处理的低分辨率图片、GIF，或低频蓝色 shader 生成层构成，前景由深色系统栏、窗口、图标和白色 Pen Pen 组成。画面可以有颗粒、色阶和像素块，但不靠装饰堆叠气氛。
...
1. **背景素材层**：全屏铺开的蓝调像素图片、GIF 或低频 shader，负责空间、时间和情绪，不承载按钮或状态信息。
...
- 背景采用经过预处理的像素图像、GIF 或低频 shader；图像默认使用 `object-fit: cover`，shader 使用同一背景层的 canvas 自适应尺寸。
...
背景不再由浏览器运行时把图片转换成 ASCII，也不通过鼠标移动实时扭曲背景。新的素材与渲染流程是：

1. 将原始图片或 GIF 放入背景素材工作区。
2. 在进入项目之前完成蓝调、像素化、色阶和尺寸适配等处理。
3. 将项目可直接使用的 PNG 或 GIF 放入 [`assets/background/`](./assets/background/)。
4. 在 [`scripts/environment/background/background-assets.js`](./scripts/environment/background/background-assets.js) 注册当前素材或 shader 描述。
5. 由背景控制器加载图片元素或单一 shader canvas：图片通过 `object-fit: cover` 全屏展示，shader 通过画布尺寸适配视口。

静态图片是默认选择；GIF 可以通过素材本身提供预制动画。当前桌面使用 `blue-fluid-halftone` shader，以低频流体场和很弱的半色调纹理提供克制的环境运动；它只读取时钟、视口和桌面运动状态，不读取鼠标位置，也不叠加粒子层。
...
- 背景默认安静；GIF 的运动来自素材本身，shader 只允许低频、低对比度的连续变化。运行时背景最多使用一个图片元素或一个 shader canvas，不引入 ASCII 字符渲染、鼠标水波、pointermove 扰动、实时滤镜、粒子层或高频持续漂移。
...
- 把 ASCII 字符矩阵、运行时鼠标水波或高频 Canvas 扰动重新引入桌面背景；低频、无指针输入的背景 shader 是受控例外。
...
- [ ] 背景是独立的 PNG / GIF 素材或已注册的 shader 描述；图片通过 `cover`、shader 通过 canvas 尺寸自适应。
- [ ] 没有运行时 ASCII、鼠标水波或高频背景扰动；shader 仅使用低频、低对比度运动。
