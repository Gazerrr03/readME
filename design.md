...
- BOT 的人格、对话结构与完整状态机。
- 新的系统级显示效果，但它们必须由琥珀与深夜两色构成，并具备明确的“设备输出”语义。
...
| Token | 值 | 用途 |
| --- | --- | --- |
| `--amber` | `#FFB454` | 文字、边框、主状态、标题栏、硬阴影 |
| `--night` | `#0A192F` | 页面底色、窗口底色、反相文字、图标填充 |

琥珀与深夜二色是品牌识别，不为新模块新增独立强调色。弱化层级时，只允许使用琥珀透明度或琥珀与深夜的混合，例如背景网格使用约 6% 琥珀，禁用态和次要信息通过透明度、线型或字号区分。

### 颜色使用规则

- 默认状态：深夜底琥珀字、琥珀色 1px 边框。
- 强调 / 选中 / 当前状态：琥珀底深夜字。
- 聚焦：`2px solid var(--amber)`，通常配合 `2px` 外偏移。
- 危险操作不引入红色；通过清晰标签、位置、确认流程或更强的琥珀与深夜反相表达。
- 图像内容允许保留原色，但系统 chrome、按钮、标签和状态指示器仍保持琥珀与深夜。
...
- 基础描边为 `1px solid var(--amber)`。
- 需要章节强调时可使用 `4px double var(--amber)`。
- 次级分隔可用 `1px dotted var(--amber)`；最小化等非活跃状态可用 dashed border。
- 不使用药丸、圆形浮动按钮或大圆角容器。
...
阴影必须是无模糊、无扩散的纯琥珀色块。按下状态可把偏移压缩到 `1px`，模拟明确的机械反馈。不要用软阴影制造悬浮卡片。

## 7. 图标与图形资产

- 系统图标以 `46 x 46px` 为标准画布，深夜底、1px 琥珀框、`4px` 琥珀色硬阴影。
...
- 选中时图标和标签同时反相：琥珀底深夜色图形 / 文字。
- 照片、视频和项目截图属于内容资产，应显示真实作品，不做模糊氛围背景；必要时用琥珀色边框或系统标签把它们纳入界面。
...
- 深夜底、1px 琥珀框、普通 `6px` 硬阴影；活动窗口提升为 `9px`。
- 标题栏高 `32px`，琥珀底深夜字；控件至少 `32 x 32px`，深夜底琥珀色符号。
...
- 当前项用琥珀底深夜字反相，不依赖下划线或颜色微差。
- Select、range、checkbox 使用原生语义并以琥珀色 accent 统一。
...
- 文字与背景保持琥珀与深夜的高对比；通过透明度弱化的内容仍需满足可读性。
...
- [ ] 仅使用既有琥珀 / 深夜 token，或有充分理由把原色媒体作为内容展示。
- [ ] 所有系统容器保持直角、1px 琥珀框和规定的硬阴影层级。
- [ ] Serif / Mono 的角色没有混用。
- [ ] 复用了现有间距、标题栏、控件和状态模式。
