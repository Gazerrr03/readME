...
test('live preview updates an open reader without losing site interactions', async ({ page }) => {
  ...
  await preview.locator('[data-windows-icons] [data-app-icon="writing"]').dblclick();
  const writingWindow = preview.locator('[data-app-window="writing"]');
  await writingWindow.locator('[data-writing-open]').first().click();
  const reader = preview.locator('[data-writing-reader]');
  ...
  await expect(reader.locator('[data-writing-title]')).toHaveText('信息开始替我思考（审校版）');

  await preview.locator('[data-content-return]').click();
  await expect(preview.locator('[data-app-window="writing"]')).toBeVisible();
  await preview.locator('[data-windows-icons] [data-app-icon="projects"]').dblclick();
  ...
  await preview.locator('[data-windows-icons] [data-app-icon="settings"]').dblclick();
