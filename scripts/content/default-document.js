...
export function createDefaultContentDocument() {
  ...
  Object.keys(dictionaries.en).forEach((key) => {
    ...
    if (LOCALES.every((locale) => values[locale] === '')) return;
  ...
  about.experience.forEach((entry, index) => {
    const prefix = `about.experience.entry-${String(index + 1).padStart(2, '0')}`;
    add(fields, `${prefix}.label`, `经历 ${index + 1} · 标签`, 'about', order++, localized(entry.label));
    add(fields, `${prefix}.name`, `经历 ${index + 1} · 名称`, 'about', order++, literal(entry.name));
  });
  about.works.forEach((entry, index) => {
    const prefix = `about.works.entry-${String(index + 1).padStart(2, '0')}`;
    add(fields, `${prefix}.name`, `作品 ${index + 1} · 名称`, 'about', order++, literal(entry.name));
    add(fields, `${prefix}.meta`, `作品 ${index + 1} · 元信息`, 'about', order++, localized(entry.meta));
    add(fields, `${prefix}.description`, `作品 ${index + 1} · 描述`, 'about', order++, localized(entry.description), 'longText');
  });
  about.toolbox.forEach((value, index) => {
    add(fields, `about.toolbox.item-${String(index + 1).padStart(2, '0')}`, `工具箱 ${index + 1}`, 'about', order++, localized(value));
  });
  about.now.forEach((entry, index) => {
    const prefix = `about.now.entry-${String(index + 1).padStart(2, '0')}`;
    add(fields, `${prefix}.key`, `当前 ${index + 1} · 标签`, 'about', order++, localized(entry.key));
